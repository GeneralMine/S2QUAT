	...
	<div class="tableOptions">
		...
		<div class="tableOptionsCreate">
			{#if enableCreateButton}
				<Button
					title={'Create ' + capitalizeFirstLetter(parseSingular(entity))}
					on:click={createButtonPressed(entity)}
				/>
			{/if}
		</div>
	</div>
							...
							<td style="cursor: pointer;" onclick="window.location='{entity + '/' + item.id}';">
								{#if attribute.type === 'img' || attribute.type === 'table'}
									<p class="text-left">
										<img
											...
											src={'entityFiles/' + attribute.name + 's/' + item[attribute.name]}
											alt={item[attribute.name]}
										/>
									</p>
								{:else if attribute.type === 'relationLink' && item[attribute.name] !== null}
									<a href={parsePlural(attribute.name) + '/' + item[attribute.name].id}
										>{attribute.name + item[attribute.name].id}</a
									>
								...
								{:else if attribute.type === 'relationLogo' && item[attribute.name] !== null}
									<p class="text-left">
										<img
											style="height: 25px; width: auto;object-fit: contain;"
											src={'entityFiles/logos/' + item[attribute.name].logo}
											alt={item[attribute.name].name}
										/>
									</p>
								{:else}
									<div>
