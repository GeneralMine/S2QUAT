...
<div class="page">
    <CardRow>
        ...
        <PieCard title="Status" data={Object.values(cardFunctions.countBy(entities, "status"))} labels={Object.keys(cardFunctions.countBy(entities, "status"))} icon="status" />
        <PieCard title="Actions" data={Object.values(cardFunctions.countBy(entities, "action"))} labels={Object.keys(cardFunctions.countBy(entities, "action"))} icon="actions" />
    </CardRow>
    <EntityView {entity} {entities} enableCreateButton={false} entitiesAttributes={listAttributes} on:update={reloadData} />
</div>
