<script context="module">
    export async function preload({ params }, session) {
        if (params !== undefined) {
            return { id: params.id };
        }
    }
</script>

<script>
    /********************************************
     * Getting env variables from session store */
    import { stores } from "@sapper/app";
    const { session } = stores();
    $: BACKEND_URL = $session.BACKEND_URL;
    /*******************************************/
    import EntityDetails from "../../components/entities/EntityDetails.svelte";
    import CardRow from "../../components/Cards/CardRow.svelte";
    import NumberCard from "../../components/Cards/NumberCard.svelte";
    import PieCard from "../../components/Cards/PieCard.svelte";
    import { postData, getData, capitalizeFirstLetter } from "../../lib";
    import cardFunctions from "../../lib/cardFunctions";
    import { onMount, onDestroy } from "svelte";
    import EntityModels from "../../lib/entityModels";

    export let id;
    let entity = "models";
    let entityObject;
    let { categories } = EntityModels[entity];

    onMount(async () => {
        await reloadData();
    });

    async function reloadData() {
        const url = BACKEND_URL + "/" + entity + "/" + id;
        console.log("Get details with", url);
        entityObject = await (await getData(url)).json();
        console.log("Loaded:", entityObject);
    }
</script>

<svelte:head>
    <title>{capitalizeFirstLetter(entity)} - S2QUAT</title>
</svelte:head>

<div>
    <CardRow>
        <NumberCard title="Questions" value={cardFunctions.lengthOfKey(entityObject, "questions")} icon="questions" />
        <NumberCard
            title="Bewertungsfelder"
            value={entityObject !== undefined ? (entityObject["questions"] !== undefined ? entityObject["questions"].filter((el) => el.depth === 0).length : 0) : 0}
            icon="amount"
        />
        <NumberCard
            title="Qualitätsmerkmale"
            value={entityObject !== undefined ? (entityObject["questions"] !== undefined ? entityObject["questions"].filter((el) => el.depth === 1).length : 0) : 0}
            icon="amount"
        />
        <NumberCard
            title="Qualitätsbestimmende Faktoren"
            value={entityObject !== undefined ? (entityObject["questions"] !== undefined ? entityObject["questions"].filter((el) => el.depth === 2).length : 0) : 0}
            icon="amount"
        />
        <NumberCard
            title="Bewertungskriterien"
            value={entityObject !== undefined ? (entityObject["questions"] !== undefined ? entityObject["questions"].filter((el) => el.depth === 3).length : 0) : 0}
            icon="amount"
        />
    </CardRow>
    <EntityDetails {entity} {entityObject} {categories} on:update={reloadData} />
</div>
